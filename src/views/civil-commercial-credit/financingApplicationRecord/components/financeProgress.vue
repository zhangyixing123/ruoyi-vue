  ...
  <div class="progress">
    ...
    <div class="flex-shrink-0">
      <el-row :gutter="10" class="mb8 flex-shrink-0">
        ...
        <el-col :span="1.5">
          <el-button type="success" plain icon="el-icon-edit" size="mini" @click="exportFun">
            导出
          </el-button>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    getList() {
      ...
      listFnApplPageType({ query, queryType: '2' }).then(res => {
    ...
    refreshLendingStatus() {
      ...
      queryOneLoanStatus({ inquiryNo }).then(res => {
        if (res.success) {
          this.msgSuccess('刷新放款状态成功！')
          this.getList()
        }
      })
    },
    ...
    exportFun() {
      ...
      exportFinance(query).then(res => {
        const a = document.createElement('a')
        const href = window.URL.createObjectURL(res)
        a.href = href
        a.download = '融资进度.xlsx'
        a.click()
