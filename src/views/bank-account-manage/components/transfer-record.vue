...
<script>
import { getTransferRecordApi } from '@/api/amount-management/index'
export default {
  ...
  methods: {
    getList() {
      ...
      if (this.tradeTime) {
        this.queryParams.tradeTimeOfBegin = this.tradeTime[0]
        this.queryParams.tradeTimeOfEnd = this.tradeTime[1]
      }
      getTransferRecordApi(this.queryParams).then(res => {
        if (res.code == 200) {
